...
// Aletheia AI Icon
function AletheiaIcon({ className }: { className?: string }) {
  return (
...
const navItems = [
  ...
  { href: "/aletheia", label: "Aletheia AI", icon: AletheiaIcon }, // 👈 ADD THIS
...
export default function Sidebar() {
  ...
  return (
    <aside
      suppressHydrationWarning
      className={`flex flex-col bg-[var(--sidebar-bg)] text-white transition-all duration-300 ease-in-out ${collapsed ? "w-20" : "w-64"
        }`}
    >
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${isActive ? "bg-[var(--sidebar-active)] text-white" : "text-white/80 hover:bg-white/10 hover:text-white"
